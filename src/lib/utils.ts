// MMSI to country flag mapping (MID codes)
const midToCountry: Record<string, string> = {
  ...
  "466": "QA", "468": "SY", "470": "AE", "472": "TJ", "473": "YE",
...
export function mmsiToFlag(mmsi: number): string {
  const mid = String(mmsi).slice(0, 3);
  const cc = midToCountry[mid];
  if (!cc) return "";
...
export function formatCoord(val: number, type: "lat" | "lon"): string {
